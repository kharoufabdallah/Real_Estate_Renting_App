...
public class SharedPrefManager {
    ...
    private SharedPrefManager(Context context) {
        sharedPreferences = context.getSharedPreferences(SHARED_PREF_NAME,
                SHARED_PREF_PRIVATE); // initialize shared preference
        editor = sharedPreferences.edit(); // initialize editor
    }
    public boolean writeString(String key, String value) { // write on sp
        editor.putString(key, value);
        return editor.commit();
    }
    public String readString(String key, String defaultValue) { // read from sp
